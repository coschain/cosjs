...
class Wallet {
  ...
  reply(sender, parent_uuid, content) {
    ...
    return _asyncToGenerator(function* () {
      const rop = new _cosGrpcJs.operation.reply_operation();
      const senderAccount = new _cosGrpcJs.raw_type.account_name();
      senderAccount.setValue(sender);
      ...
      const signTx = yield _this16.signOps(sender, [rop]);
  ...
  vote(voter, idx) {
    ...
    return _asyncToGenerator(function* () {
      const vop = new _cosGrpcJs.operation.vote_operation();
      const voterAccount = new _cosGrpcJs.raw_type.account_name();
      voterAccount.setValue(voter);
      vop.setVoter(voterAccount);
      vop.setIdx(idx);
      const signTx = yield _this17.signOps(voter, [vop]);
  ...
  contractCall(caller, owner, contract, method, args, payment) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const signTx = yield _this18.signOps(caller, [callOp]);
      return _this18.broadcast(signTx);
    })();
  }
  ...
  voteToBlockProducer(voterValue, bpValue, cancel) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const signTx = yield _this19.signOps(voterValue, [bpVoteOp]);
      return _this19.broadcast(signTx);
    })();
  ...
  queryTable(owner, contract, table, field, begin, limit, reverse) {
    ...
    return _asyncToGenerator(function* () {
      ...
      return new Promise(resolve => _this20.cos.grpc.unary(ApiService.QueryTableContent, {
        request: getTableContentRequest,
        host: _this20.cos.provider,
        onEnd: res => {
          const {
            status,
            statusMessage,
          ...
          if (status === _this20.cos.grpc.Code.OK && message) {
            let object = message.toObject();
            resolve(object);
          } else {
            resolve({
  ...
  delegateVest(sender, receiver, amount, expiration) {
    var _this21 = this;

    return _asyncToGenerator(function* () {
      let op = new _cosGrpcJs.operation.delegate_vest_operation();
      let fromAccount = new _cosGrpcJs.raw_type.account_name();
      fromAccount.setValue(sender);
      op.setFrom(fromAccount);
      let toAccount = new _cosGrpcJs.raw_type.account_name();
      toAccount.setValue(receiver);
      op.setTo(toAccount);
      let value = util.parseIntoNumber(amount);
      let vest = new _cosGrpcJs.raw_type.vest();
      vest.setValue(value.toString());
      op.setAmount(vest);
      op.setExpiration(expiration);
      const signTx = yield _this21.signOps(sender, [op]);
      return _this21.broadcast(signTx);
    })();
  }
  ...
  unDelegateVest(sender, orderId) {
    ...
    return _asyncToGenerator(function* () {
      let op = new _cosGrpcJs.operation.un_delegate_vest_operation();
      let accountName = new _cosGrpcJs.raw_type.account_name();
      accountName.setValue(sender);
      op.setAccount(accountName);
      op.setOrderId(orderId);
      const signTx = yield _this22.signOps(sender, [op]);
      return _this22.broadcast(signTx);
    })();
  }

  vestDelegationOrderList(account, limit, is_lender) {
    var _this23 = this;

    return _asyncToGenerator(function* () {
      let req = new _cosGrpcJs.grpc.GetVestDelegationOrderListRequest();
      let accountName = new _cosGrpcJs.raw_type.account_name();
      ...
      req.setLimit(limit);
      return new Promise(resolve => _this23.cos.grpc.unary(ApiService.GetVestDelegationOrderList, {
        request: req,
        host: _this23.cos.provider,
        onEnd: res => {
          const {
            status,
            statusMessage,
          ...
          if (status === _this23.cos.grpc.Code.OK && message) {
            let object = message.toObject();
            resolve(object);
          } else {
            resolve({
